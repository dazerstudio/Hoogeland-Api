...
export default async function handler(req, res) {
  ...
  const data = await page.evaluate(async (date) => {
    // Stupid typescript shut up, this isn't run in this scope
    ...
    if(date != null) {
      ...
      const wait = (resolve) => {
        if(angular.element(document.querySelector(
            '#afsprakenLijst > div.k-grid-content > table > tbody > .k-grouping-row.ng-scope'
        )).scope().dataItem.value.toDateString() != start) resolve();
    ...
    data = data.map((day) => {
      ...
      parsedDay.subjects = parsedDay.subjects.map((subject) => ({
